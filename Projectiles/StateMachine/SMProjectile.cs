...
namespace SkyHell.Projectiles.StateMachine
{
    ...
    public abstract class SMProjectile : ModProjectile
    {
        ...
        public void SetState<T>() where T : ProjState
        {
            ...
            {
                throw new ArgumentException("这个状态并不存在");
            }
        ...
        {
            var name = typeof(T).FullName;
            return !stateDict.ContainsKey(name) ? throw new ArgumentException("这个状态并不存在") : State == stateDict[name];
        }
        /// <summary>
        /// 注册状态
        /// </summary>
        ...
        protected void RegisterState<T>(T state) where T : ProjState
        {
            ...
            if (stateDict.ContainsKey(name))
            {
                throw new ArgumentException("这个状态已经注册过了");
            }
